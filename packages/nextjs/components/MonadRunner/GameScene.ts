// monad-app/packages/nextjs/components/MonadRunner/GameScene.ts
...
export class GameScene extends ex.Scene {
  ...
  public onInitialize(engine: ex.Engine) {
    ...
    if (!this.isReplayMode) {
      ...
      engine.input.keyboard.on("release", (evt) => {
        ...
        if ((evt.key === ex.Keys.A || evt.key === ex.Keys.Left) && this.player.vel.x < 0) {
          this.player.vel.x = 0;
  ...
  private killPlayer(engine: ex.Engine) {
    if (this.gameOver) {
      console.log("[GameScene] killPlayer: Already killed, skipping.");
      return;
    }
    this.gameOver = true;
    this.player.kill();
    ...
    if (!this.isReplayMode) {
      ...
      window.dispatchEvent(new CustomEvent("replaydata", { detail: { replay: replayData } }));
    }
