...
export async function GET(req: NextRequest) {
  const requestId = `getallreplays-${Date.now()}-${Math.random().toString(36).substring(7)}`;
  console.log(`[Replay GET API ${requestId}] Fetching all replays`);

  try {
    // Forward the request to your backend endpoint for all replays
    const response = await fetch(`${API_URL}/runnerapi/game/replays`, {
      method: "GET",
      headers: {
